...
def build_model(max_tokens=23, hidden_dim=64):
    ...
    x = Dropout(0.5)(x)
    x = Flatten()(x)
    outputs = Dense(2, activation="sigmoid")(x) # binary activation output(sigmoid:1,softmax":2))
...
#            print("SearchTowerdsSparrow")
#            print(len(new_weights[rn]))
...
class OptimizerSSA:
    def __init__(self,model, max_tokens):
        ...
        for i in range(self.n_sparrows):
            self.sparrows[i] = Sparrow(max_tokens =max_tokens )
